...
export default function useTrimData(course, windSpeed, waveHeight) {
  ...
  useEffect(() => {
    getHeadSailTrim(course, windSpeed, waveHeight).then((data) =>
      setHeadSailTrimData(data)
    );
  }, [course, waveHeight, windSpeed]);

  useEffect(() => {
    getMainSailTrim(course, windSpeed, waveHeight).then((data) =>
      setMainSailTrimData(data)
    );
  }, [course, waveHeight, windSpeed]);
