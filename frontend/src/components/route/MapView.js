                ...
                <GoogleMap mapContainerStyle={mapContainerStyle} zoom={10} center={center} options={options}>
                    {routing.map((leg) =>
                        <Marker
                            ...
                            onClick={() => {setSelected({leg: leg, index: routing.indexOf(leg)})}}
                        />
                    )}
                    <Polyline path={routing}/>
                    {selected ?
                    <InfoWindow position={selected.leg} onCloseClick={() => {setSelected(null)}}>
                        <div>
                            <h2>{nameWaypoint(selected?.index)}</h2>
                            <p>Latitude: {selected?.leg.lat}</p><p>Longitude: {selected?.leg.lng}</p>
                        </div>
                    </InfoWindow> : null}
                </GoogleMap>
